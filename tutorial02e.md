...
We'll start out where we ended the previous session, but we'll make some changes right off the bat: 
We're going to distribute the energy unevenly, like in the real world. One island will have a lot of energy, the other island will have much less.

### Step 2
We'll start by increasing the odds for the energy sprites to be placed on the sandy island. 
Into the ``||loops.Repeat||`` loop we'll place an 
 ``||Logic:if true then else ||`` block from the ``||Logic.Logic||`` menu. 
 Place it underneath the ``||variables. set energy to..||`` block.

Tip: To move a single block, hold down Ctrl while clicking and dragging the block you want to move.
...
### Step 5
Whatever is put in ``||Logic.else||`` will have a 20 % probability of occuring. 
Copy ``||scene.place energy on top of random ||`` and place into the ``||logic.else||`` gap.
Click the sand coloured square and change the tile to the green one. 
...
### Step 6
Now we're going to place ``||sprite.mySprite||`` in a similar fashion. 
We want a 20 % probability of starting the game on the sandy island, and an 80 % probability of starting on the green island. 

Find a new ``||Logic.if true then else||`` block, and place it underneath
...
### Step 7
Move and copy ``||scene.place mySprite on top of random...||`` 
into the ``||logic. if true then else ||`` block so you'll have a 20 % 
probability for the player character being placed on the sandy island,
and an 80 % probability for them being placed on the green island. 
...
### Step 8
We'll finish off by setting a condition for winning the game. If you score 20 points or more, you win.
Start by placing a ``||info. onCountdownEnd||`` block from the ``||info.info||`` menu anywhere on the screen. 
Place an ``||logic.if true then else||`` block inside it. 
...
### Step 9
Where it says ``||logic.true||`` we're now going to compare two numbers. 
In the ``||logic.logic||`` menu you'll find a ``||logic. 0 < 0||`` block. Place this where it says ``||logic.true||``

Get the ``||info.score||`` variable from the ``||info.info||`` menu and 
place this as the first number in the ``||logic. 0 < 0||`` block. 
Change the other 0 to 20.
...
```
### Steg 10
Now all we need are two ``||game.game over||`` blocks 
from the ``||game.game||`` menu. Place one in the gap under ``||logic.if||`` 
an one in the gap under ``||logic.else||``. 

Which one should be set to ``||game.win||`` an which one shold be set to ``||game.lose||`` ? 
...
Well done! You're well on your way to coding your own game! 😊
How many points can you score, and who's the best player in your class?  

What else would you like to add to your game?
